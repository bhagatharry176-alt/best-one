...
from modules.ott_downloader import ott_super_command, ott_callback_handler
...
from pyrogram.errors import FloodWait, PeerIdInvalid, UserIsBlocked, InputUserDeactivated, StickerEmojiInvalid
...
# Initialize the bot with ULTRA FAST upload optimization
bot = Client(
    "bot",
    api_id=API_ID,
    api_hash=API_HASH,
    bot_token=BOT_TOKEN,
    workers=12,                   # Maximum workers for ultra speed
    max_concurrent_transmissions=8,  # Maximum concurrent uploads for speed
    session_string=None,           # Force new session creation
...
# Upload lock for ultra-fast single file uploads
upload_lock = False
import shutil
import tempfile

async def ultra_fast_upload(file_path: str, chat_id: int, caption: str = None, upload_type: str = "document"):
    """
    ULTRA FAST upload function - moves files to SSD-backed /tmp and uploads without progress callbacks
    """
    global upload_lock
    
    # Wait if another upload is in progress (prevent bandwidth contention)
    while upload_lock:
    ...
    try:
        # Step 1: Move file to SSD-backed /tmp for ultra-fast disk I/O
        file_name = os.path.basename(file_path)
        temp_file = f"/tmp/ultra_fast_{int(time.time())}_{file_name}"
        
        # Fast file move/copy to /tmp
        if os.path.exists(file_path):
            shutil.move(file_path, temp_file)
            print(f"🚀 ULTRA FAST: Moved {file_name} to SSD /tmp for fast upload")
        else:
            upload_lock = False
            return None
        
        # Step 2: Ultra-fast upload WITHOUT progress callbacks (eliminates RPC bottleneck)
        ...
        if upload_type == "video":
            result = await bot.send_video(
                ...
                video=temp_file,
                caption=caption,
                # NO progress callback = maximum speed
            )
        else:
        ...
        print(f"⚡ ULTRA FAST UPLOAD: {file_size:.1f}MB in {upload_time:.1f}s = {speed:.2f} MB/s")
        ...
        # Cleanup temp file and release lock
        upload_lock = False
...
async def update_download_progress(download_id, safe_filename, user_id, progress, message, completed=False):
    """
    Update download progress for user - used by drm_handler for file downloads
    
    Args:
        download_id: Unique identifier for the download
        safe_filename: Name of the file being downloaded  
        user_id: User who initiated the download
        progress: Progress percentage (0-100)
        message: Status message to show user
        completed: Whether download is finished
    """
    try:
        if completed:
            # Send final status message
            if progress == 100:
                status_text = f"📁 **{safe_filename}**\n\n✅ {message}"
            else:
                status_text = f"📁 **{safe_filename}**\n\n❌ {message}"
            ...
            # Store progress message for potential updates
            if download_id not in active_downloads:
                progress_msg = await bot.send_message(user_id, status_text)
                active_downloads[download_id] = progress_msg
            else:
                # Update existing progress message
                try:
                    await active_downloads[download_id].edit_text(status_text)
                except Exception:
...
async def start(bot, m: Message):
    ...
    async def safe_edit_text(message, text):
        """Safely edit message text to prevent MESSAGE_NOT_MODIFIED errors"""
        try:
            await message.edit_text(text)
        except Exception as e:
            if "MESSAGE_NOT_MODIFIED" in str(e):
                # Message content is the same, skip this edit
                pass
            else:
    ...
    await safe_edit_text(start_message,
    ...
    if m.chat.id in AUTH_USERS:
        ...
        # Apply keyboard and disable_web_page_preview separately to avoid conflicts
        try:
            await start_message.edit_reply_markup(reply_markup=keyboard)
        except Exception:
            pass
    else:
...
async def settings_button(client, callback_query):
    user_id = callback_query.from_user.id
    if user_id != OWNER and user_id not in AUTH_USERS:
        await callback_query.answer("❌ Unauthorized access! Contact admin for access.", show_alert=True)
        return
...
async def handle_token(client, callback_query):
    ...
    try:
        ...
        await editable.edit(f"✅ Classplus Token set successfully !\n\n<blockquote expandable>Token has been securely stored and will be used for authentication.</blockquote>", reply_markup=keyboard)
...
# .....,.....,.......,...,.......,....., .....,.....,.......,...,.......,.....,
@bot.on_message(filters.command("ott") & filters.private)
async def call_ott_super_command(bot: Client, m: Message):
    await ott_super_command(bot, m)

# .....,.....,.......,...,.......,....., .....,.....,.......,...,.......,.....,
@bot.on_callback_query(filters.regex(r"^ott_|^back_to_ott$|^revoke_"))
async def call_ott_callback_handler(bot: Client, callback_query):
    await ott_callback_handler(bot, callback_query)

# .....,.....,.......,...,.......,....., .....,.....,.......,...,.......,.....,
@bot.on_message(filters.command("kk") & filters.private)
async def kk_command(bot: Client, m: Message):
    if m.chat.id not in AUTH_USERS:
        await m.reply_text(f"<blockquote>__**Oopss! You are not authorized\nSend me your user id for authorization\nYour User id**__ - `{m.chat.id}`</blockquote>")
        return
    
    await m.reply_text(
        "🎯 **KK Command - Quick Actions**\n\n"
        "This is a premium shortcut command for:\n"
        "• Quick file processing\n"
        "• Batch operations\n"
        "• Advanced settings\n\n"
        "**Usage:** Send any file with /kk to process it quickly!"
    )
...
def reset_and_set_commands():
    try:
        ...
        commands = [
            ...
            {"command": "ott", "description": "🎭 OTT Platform Downloader"},
            {"command": "kk", "description": "🎯 Quick Actions & Shortcuts"},
            {"command": "cookies", "description": "📁 Upload YT Cookies"},
            {"command": "getcookies", "description": "📤 Download YT Cookies"},
...
if __name__ == "__main__":
    print("🚀 Starting SAINI DRM Bot on Replit...")
    ...
    try:
        ...
        print("✅ Cleared webhook, using polling mode")
    except Exception as e:
